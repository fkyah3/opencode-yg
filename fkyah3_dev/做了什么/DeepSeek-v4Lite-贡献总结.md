...
**发现**：fork 从 upstream 复制了 373 个无关分支。用户希望仓库只保留我们自己的分支，让首页干净可读。

**做了什么**：
- **风险评估**：这是一个高风险操作——373 次 API 删除调用，超时风险，误删则无法恢复
- **安全策略**：先创建 `main` 分支并推送到远程 → 切换默认分支到 `main`（确保仓库不因默认分支被删而崩溃）→ 再用白名单过滤删除
- **分批执行**：第一批 ~250 个（5 分钟超时截断）→ 验证 → 第二批删除剩余 125 个 → 再验证
- **白名单保护**：仅保留 `main` 和 `fix/permission-reasoning-truncation`，其余全部删除
- **本地收尾**：清理孤儿本地分支（`dev`、`pr/cjk-encoding-fix`），prune remote-tracking refs
- **文档**：在 README 和 ACHIEVEMENTS.md 记录操作过程和风险控制策略，供后续参考

**涉及文件**：
- `README.md`（根目录——重写为 fork 首页，让来人一眼看到这是谁的 fork）
- `fkyah3_dev/README.md`（新增 §4 仓库清理说明）
- `fkyah3_dev/ACHIEVEMENTS.md`（新增条目 #4）

**难度**：⭐⭐⭐（较高风险—远程批量操作，需要分层保护和验证策略）
