## NavigationPathQueryParameters2D（2D导航路径查询参数）<- RefCounted（引用计数）

通过更改此对象的各个属性（如起点和目标位置），可以配置向 NavigationServer2D 的路径查询。

**属性（Props）：**
- excluded_regions: RID[] = [] —— 排除的区域
- included_regions: RID[] = [] —— 包含的区域
- map: RID = RID() —— 导航地图
- metadata_flags: int (NavigationPathQueryParameters2D.PathMetadataFlags) = 7 —— 元数据标志
