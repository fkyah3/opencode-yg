## NavigationServer2D（2D导航服务器）<- Object（对象）

NavigationServer2D 是处理导航地图、区域和代理的服务器。它不处理来自 AStar2D 或 AStarGrid2D 的 A* 导航。地图被划分为多个区域，区域由导航多边形组成。它们共同定义了 2D 世界中的可通行区域。**注意：** 大多数 NavigationServer2D 的更改会在下一个物理帧后生效，而不是立即生效。这包括场景树中导航相关节点或通过脚本对地图、区域或代理所做的所有更改。两个区域要相互连接，必须共享一条相似的边。如果一条边的两个顶点与另一条边的对应顶点之间的距离小于 `edge_connection_margin`，则认为这两条边已连接。可以使用 `NavigationServer2D.region_set_navigation_layers` 为区域分配导航层，然后在通过 `NavigationServer2D.map_get_path` 请求路径时进行检查。这可用于允许或拒绝某些物体进入特定区域。要使用碰撞避免系统，可以使用代理。您可以设置代理的目标速度，然后服务器将发出带有修改后速度的回调。**注意：** 碰撞避免系统会忽略区域。直接使用修改后的速度可能会将代理移动到可通行区域之外。这是碰撞避免系统的限制，任何更复杂的情况可能需要使用物理引擎。此服务器会跟踪所有调用并在同步阶段执行它们。这意味着您可以在任何线程上请求对地图的任何更改，而无需担心。
...
- agent_get_position(agent: RID) -> Vector2 —— 获取代理位置
...
- agent_get_velocity(agent: RID) -> Vector2 —— 获取代理速度
...
- agent_set_position(agent: RID, position: Vector2) —— 设置代理位置
...
- agent_set_velocity(agent: RID, velocity: Vector2) —— 设置代理速度
- agent_set_velocity_forced(agent: RID, velocity: Vector2) —— 强制设置代理速度
- bake_from_source_geometry_data(navigation_polygon: NavigationPolygon, source_geometry_data: NavigationMeshSourceGeometryData2D, callback: Callable = Callable()) —— 从源几何数据烘焙导航多边形
- bake_from_source_geometry_data_async(navigation_polygon: NavigationPolygon, source_geometry_data: NavigationMeshSourceGeometryData2D, callback: Callable = Callable()) —— 异步从源几何数据烘焙导航多边形
- free_rid(rid: RID) —— 释放 RID
- get_debug_enabled() -> bool —— 获取调试是否启用
- get_maps() -> RID[] —— 获取所有地图
- get_process_info(process_info: int) -> int —— 获取进程信息
- is_baking_navigation_polygon(navigation_polygon: NavigationPolygon) -> bool —— 导航多边形是否正在烘焙
- link_create() -> RID —— 创建链接
- link_get_enabled(link: RID) -> bool —— 获取链接启用状态
- link_get_end_position(link: RID) -> Vector2 —— 获取链接终点位置
...
- link_get_start_position(link: RID) -> Vector2 —— 获取链接起点位置
...
- link_set_end_position(link: RID, position: Vector2) —— 设置链接终点位置
...
- link_set_start_position(link: RID, position: Vector2) —— 设置链接起点位置
...
- map_get_closest_point(map: RID, to_point: Vector2) -> Vector2 —— 获取地图上最近的点
- map_get_closest_point_owner(map: RID, to_point: Vector2) -> RID —— 获取地图上最近点的所有者
...
- map_get_path(map: RID, origin: Vector2, destination: Vector2, optimize: bool, navigation_layers: int = 1) -> PackedVector2Array —— 获取地图路径
- map_get_random_point(map: RID, navigation_layers: int, uniformly: bool) -> Vector2 —— 获取地图随机点
...
- obstacle_get_position(obstacle: RID) -> Vector2 —— 获取障碍物位置
- obstacle_get_radius(obstacle: RID) -> float —— 获取障碍物半径
- obstacle_get_velocity(obstacle: RID) -> Vector2 —— 获取障碍物速度
- obstacle_get_vertices(obstacle: RID) -> PackedVector2Array —— 获取障碍物顶点
...
- obstacle_set_position(obstacle: RID, position: Vector2) —— 设置障碍物位置
- obstacle_set_radius(obstacle: RID, radius: float) —— 设置障碍物半径
- obstacle_set_velocity(obstacle: RID, velocity: Vector2) —— 设置障碍物速度
- obstacle_set_vertices(obstacle: RID, vertices: PackedVector2Array) —— 设置障碍物顶点
- parse_source_geometry_data(navigation_polygon: NavigationPolygon, source_geometry_data: NavigationMeshSourceGeometryData2D, root_node: Node, callback: Callable = Callable()) —— 解析源几何数据
- query_path(parameters: NavigationPathQueryParameters2D, result: NavigationPathQueryResult2D, callback: Callable = Callable()) —— 查询路径
- region_create() -> RID —— 创建区域
- region_get_bounds(region: RID) -> Rect2 —— 获取区域边界
- region_get_closest_point(region: RID, to_point: Vector2) -> Vector2 —— 获取区域中最近的点
- region_get_connection_pathway_end(region: RID, connection: int) -> Vector2 —— 获取区域连接的终点
- region_get_connection_pathway_start(region: RID, connection: int) -> Vector2 —— 获取区域连接的起点
...
- region_get_random_point(region: RID, navigation_layers: int, uniformly: bool) -> Vector2 —— 获取区域随机点
- region_get_transform(region: RID) -> Transform2D —— 获取区域变换
...
- region_owns_point(region: RID, point: Vector2) -> bool —— 区域是否拥有该点
...
- region_set_navigation_polygon(region: RID, navigation_polygon: NavigationPolygon) —— 设置区域导航多边形
- region_set_owner_id(region: RID, owner_id: int) —— 设置区域所有者 ID
- region_set_transform(region: RID, transform: Transform2D) —— 设置区域变换
...
- simplify_path(path: PackedVector2Array, epsilon: float) -> PackedVector2Array —— 简化路径
