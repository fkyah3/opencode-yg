## CPUParticles3D（CPU粒子3D）<- GeometryInstance3D（几何体实例3D）

基于 CPU 的 3D 粒子节点，用于创建各种粒子系统和效果。另请参见 GPUParticles3D，它提供相同但带有硬件加速的功能，可能不适用于较旧的设备。
...
- draw_order: int (CPUParticles3D.DrawOrder) = 0 —— 绘制顺序
- emission_box_extents: Vector3 —— 发射盒范围
- emission_colors: PackedColorArray = PackedColorArray() —— 发射颜色
- emission_normals: PackedVector3Array —— 发射法线
- emission_points: PackedVector3Array —— 发射点
...
- orbit_velocity_max: float —— 最大轨道速度
- orbit_velocity_min: float —— 最小轨道速度
- particle_flag_align_y: bool = false —— 粒子标志 Y 轴对齐
- particle_flag_disable_z: bool = false —— 粒子标志禁用 Z
- particle_flag_rotate_y: bool = false —— 粒子标志绕 Y 轴旋转
...
- visibility_aabb: AABB = AABB(0, 0, 0, 0, 0, 0) —— 可见性包围盒

**方法（Methods）：**
- capture_aabb() -> AABB —— 捕获包围盒
...
**枚举（Enums）：**
**DrawOrder（绘制顺序）：** DRAW_ORDER_INDEX=0 —— 按索引，DRAW_ORDER_LIFETIME=1 —— 按生命周期，DRAW_ORDER_VIEW_DEPTH=2 —— 按视图深度
...
**EmissionShape（发射形状）：** EMISSION_SHAPE_POINT=0 —— 点，EMISSION_SHAPE_SPHERE=1 —— 球体，EMISSION_SHAPE_SPHERE_SURFACE=2 —— 球体表面，EMISSION_SHAPE_BOX=3 —— 盒体，EMISSION_SHAPE_POINTS=4 —— 多点，EMISSION_SHAPE_DIRECTED_POINTS=5 —— 有向点，EMISSION_SHAPE_RING=6 —— 环，EMISSION_SHAPE_MAX=7
