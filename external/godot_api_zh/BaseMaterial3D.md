## BaseMaterial3D（基础材质3D）<- Material

此类作为默认材质，提供广泛的渲染功能和属性，无需编写着色器代码。详情请参阅以下教程。
...
**枚举（Enums）：**
**TextureParam（纹理参数）：** TEXTURE_ALBEDO=0, TEXTURE_METALLIC=1, TEXTURE_ROUGHNESS=2, TEXTURE_EMISSION=3, TEXTURE_NORMAL=4, TEXTURE_BENT_NORMAL=18, TEXTURE_RIM=5, TEXTURE_CLEARCOAT=6, TEXTURE_FLOWMAP=7, TEXTURE_AMBIENT_OCCLUSION=8, ...
**TextureFilter（纹理过滤）：** TEXTURE_FILTER_NEAREST=0, TEXTURE_FILTER_LINEAR=1, TEXTURE_FILTER_NEAREST_WITH_MIPMAPS=2, TEXTURE_FILTER_LINEAR_WITH_MIPMAPS=3, TEXTURE_FILTER_NEAREST_WITH_MIPMAPS_ANISOTROPIC=4, TEXTURE_FILTER_LINEAR_WITH_MIPMAPS_ANISOTROPIC=5, TEXTURE_FILTER_MAX=6
**DetailUV（细节UV）：** DETAIL_UV_1=0, DETAIL_UV_2=1
**Transparency（透明度）：** TRANSPARENCY_DISABLED=0, TRANSPARENCY_ALPHA=1, TRANSPARENCY_ALPHA_SCISSOR=2, TRANSPARENCY_ALPHA_HASH=3, TRANSPARENCY_ALPHA_DEPTH_PRE_PASS=4, TRANSPARENCY_MAX=5
**ShadingMode（着色模式）：** SHADING_MODE_UNSHADED=0, SHADING_MODE_PER_PIXEL=1, SHADING_MODE_PER_VERTEX=2, SHADING_MODE_MAX=3
**Feature（特性）：** FEATURE_EMISSION=0, FEATURE_NORMAL_MAPPING=1, FEATURE_RIM=2, FEATURE_CLEARCOAT=3, FEATURE_ANISOTROPY=4, FEATURE_AMBIENT_OCCLUSION=5, FEATURE_HEIGHT_MAPPING=6, FEATURE_SUBSURFACE_SCATTERING=7, FEATURE_SUBSURFACE_TRANSMITTANCE=8, FEATURE_BACKLIGHT=9, ...
**BlendMode（混合模式）：** BLEND_MODE_MIX=0, BLEND_MODE_ADD=1, BLEND_MODE_SUB=2, BLEND_MODE_MUL=3, BLEND_MODE_PREMULT_ALPHA=4
**AlphaAntiAliasing（Alpha抗锯齿）：** ALPHA_ANTIALIASING_OFF=0, ALPHA_ANTIALIASING_ALPHA_TO_COVERAGE=1, ALPHA_ANTIALIASING_ALPHA_TO_COVERAGE_AND_TO_ONE=2
**DepthDrawMode（深度绘制模式）：** DEPTH_DRAW_OPAQUE_ONLY=0, DEPTH_DRAW_ALWAYS=1, DEPTH_DRAW_DISABLED=2
**DepthTest（深度测试）：** DEPTH_TEST_DEFAULT=0, DEPTH_TEST_INVERTED=1
**CullMode（剔除模式）：** CULL_BACK=0, CULL_FRONT=1, CULL_DISABLED=2
**Flags（标志）：** FLAG_DISABLE_DEPTH_TEST=0, FLAG_ALBEDO_FROM_VERTEX_COLOR=1, FLAG_SRGB_VERTEX_COLOR=2, FLAG_USE_POINT_SIZE=3, FLAG_FIXED_SIZE=4, FLAG_BILLBOARD_KEEP_SCALE=5, FLAG_UV1_USE_TRIPLANAR=6, FLAG_UV2_USE_TRIPLANAR=7, FLAG_UV1_USE_WORLD_TRIPLANAR=8, FLAG_UV2_USE_WORLD_TRIPLANAR=9, ...
**DiffuseMode（漫射模式）：** DIFFUSE_BURLEY=0, DIFFUSE_LAMBERT=1, DIFFUSE_LAMBERT_WRAP=2, DIFFUSE_TOON=3
**SpecularMode（镜面反射模式）：** SPECULAR_SCHLICK_GGX=0, SPECULAR_TOON=1, SPECULAR_DISABLED=2
**BillboardMode（公告板模式）：** BILLBOARD_DISABLED=0, BILLBOARD_ENABLED=1, BILLBOARD_FIXED_Y=2, BILLBOARD_PARTICLES=3
**TextureChannel（纹理通道）：** TEXTURE_CHANNEL_RED=0, TEXTURE_CHANNEL_GREEN=1, TEXTURE_CHANNEL_BLUE=2, TEXTURE_CHANNEL_ALPHA=3, TEXTURE_CHANNEL_GRAYSCALE=4
**EmissionOperator（自发光运算）：** EMISSION_OP_ADD=0, EMISSION_OP_MULTIPLY=1
**DistanceFadeMode（距离淡出模式）：** DISTANCE_FADE_DISABLED=0, DISTANCE_FADE_PIXEL_ALPHA=1, DISTANCE_FADE_PIXEL_DITHER=2, DISTANCE_FADE_OBJECT_DITHER=3
**StencilMode（模板模式）：** STENCIL_MODE_DISABLED=0, STENCIL_MODE_OUTLINE=1, STENCIL_MODE_XRAY=2, STENCIL_MODE_CUSTOM=3
**StencilFlags（模板标志）：** STENCIL_FLAG_READ=1, STENCIL_FLAG_WRITE=2, STENCIL_FLAG_WRITE_DEPTH_FAIL=4
**StencilCompare（模板比较）：** STENCIL_COMPARE_ALWAYS=0, STENCIL_COMPARE_LESS=1, STENCIL_COMPARE_EQUAL=2, STENCIL_COMPARE_LESS_OR_EQUAL=3, STENCIL_COMPARE_GREATER=4, STENCIL_COMPARE_NOT_EQUAL=5, STENCIL_COMPARE_GREATER_OR_EQUAL=6
