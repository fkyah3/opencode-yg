...
- debug_path_custom_line_width: float = -1.0 —— 调试路径自定义线宽
...
- neighbor_distance: float = 500.0 —— 邻居检测距离
- path_desired_distance: float = 20.0 —— 路径期望距离
- path_max_distance: float = 100.0 —— 路径最大距离
- path_metadata_flags: int (NavigationPathQueryParameters2D.PathMetadataFlags) = 7 —— 路径元数据标志
- path_postprocessing: int (NavigationPathQueryParameters2D.PathPostProcessing) = 0 —— 路径后处理
...
- pathfinding_algorithm: int (NavigationPathQueryParameters2D.PathfindingAlgorithm) = 0 —— 寻路算法
- radius: float = 10.0 —— 代理半径
- simplify_epsilon: float = 0.0 —— 简化精度
- simplify_path: bool = false —— 是否简化路径
- target_desired_distance: float = 10.0 —— 目标期望距离
- target_position: Vector2 = Vector2(0, 0) —— 目标位置
- time_horizon_agents: float = 1.0 —— 代理时间视界
- time_horizon_obstacles: float = 0.0 —— 障碍物时间视界
- velocity: Vector2 = Vector2(0, 0) —— 当前速度
...
- get_current_navigation_path() -> PackedVector2Array —— 获取当前导航路径
- get_current_navigation_path_index() -> int —— 获取当前导航路径索引
- get_current_navigation_result() -> NavigationPathQueryResult2D —— 获取当前导航查询结果
- get_final_position() -> Vector2 —— 获取最终位置
- get_navigation_layer_value(layer_number: int) -> bool —— 获取导航层值
- get_navigation_map() -> RID —— 获取导航地图
- get_next_path_position() -> Vector2 —— 获取下一个路径位置
...
- set_velocity_forced(velocity: Vector2) —— 强制设置速度
...
- velocity_computed(safe_velocity: Vector2) —— 速度计算完成时触发
