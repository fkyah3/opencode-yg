...
const StatusDialog = (props: { api: TuiPluginApi; s: StatusDetail }) => {
    ...
    const breakdownSegments = () => {
        ...
        if (d.conversationTokens > 0)
            segs.push({ label: "对话", tokens: d.conversationTokens, color: t().textMuted })
        if (d.toolCallTokens > 0)
            segs.push({ label: "工具调用", tokens: d.toolCallTokens, color: "#6b7280" })
        if (d.toolDefinitionTokens > 0)
            segs.push({ label: "工具定义 + 开销", tokens: d.toolDefinitionTokens, color: "#9ca3af" })
        ...
        <box flexDirection="column" width="100%" paddingLeft={2} paddingRight={2} paddingTop={1} paddingBottom={1}>
            ...
            <box flexDirection="row" width="100%" marginTop={1} gap={4}>
                ...
                <box flexDirection="column" flexGrow={1} flexBasis={0}>
                    ...
                    <R t={t()} l="状态" v={s().pendingOpsCount > 0 ? `待清理 (${s().pendingOpsCount})` : "无"} fg={s().pendingOpsCount > 0 ? t().warning : t().textMuted} />
                    <box marginTop={1}>
                        <text fg={t().text}><b>缓存 TTL</b></text>
                    </box>
                    <R t={t()} l="配置值" v={s().cacheTtl} />
                ...
                {/* Right column */}
                <box flexDirection="column" flexGrow={1} flexBasis={0}>
                    <text fg={t().text}><b>滚动提示</b></text>
                    <R t={t()} l="执行阈值" v={`${s().executeThreshold}%`} />
                    <R t={t()} l="锚点" v={`${fmt(s().lastNudgeTokens)} tok`} />
                    <R t={t()} l="间隔" v={`${fmt(s().nudgeInterval)} tok`} fg={t().textMuted} />
                    <R t={t()} l="下次提示" v={`${fmt(s().nextNudgeAfter)} tok`} />
                    {s().lastNudgeBand ? <R t={t()} l="当前区间" v={s().lastNudgeBand} /> : null}
                    <box marginTop={1}>
                        <text fg={t().text}><b>上下文详情</b></text>
                    </box>
                    <R t={t()} l="保护标签" v={String(s().protectedTagCount)} fg={t().textMuted} />
                    <R t={t()} l="子会话" v={s().isSubagent ? "是" : "否"} fg={t().textMuted} />
                    <box marginTop={1}>
                    ...
                    {s().lastDreamerRunAt && (
                        <R t={t()} l="梦幻者" v={`上次 ${relTime(s().lastDreamerRunAt!)}`} fg={t().textMuted} />
                    )}
                </box>
            </box>
