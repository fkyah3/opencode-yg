...
func _count_fence_chars(line: String, character: String) -> int:
	## 统计剥离后的行中开头 fence 字符的数量（用于比对上下的 fence 长度）
...
func _denotes_fenced_code_block(line: String, character: String) -> bool:
	...
	if count >= 3 and stripped_line.begins_with(character.repeat(3)):
		return true
	else:
		return false
