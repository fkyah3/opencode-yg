...
var _pending_total_correction: bool = false  # 加载完成后需重算一次总高
...
func _process(delta: float) -> void:
	...
	# 首次加载完成后，用修正高度重设总高一次（只做一次）
	if _pending_total_correction:
		_pending_total_correction = false
		# 强制测量所有未渲染行
		for i in range(_row_data.size()):
			if not _row_assignments.has(i) and not _free_nodes.is_empty():
		...
		# 从修正后的高度重算总高
		if not _row_heights.is_empty():
			var total := 0.0
			for h in _row_heights:
				total += h
			virtual_content.custom_minimum_size.y = total
		# 修正完后才推底——保证 max_value 已刷新
...
func _load_session_messages(sid: String) -> void:
	...
	# 标记总高修正（_process 中先修正再推底——两帧后到底）
	_pending_total_correction = true
...
func _compute_heights_and_offsets() -> void:
	print("→ _compute_heights_and_offsets @ " + str(Time.get_ticks_msec()))
	print_stack()
	## 按内容估算每行高度，计算 Y 偏移数组
	_y_offsets.clear()
	_row_heights.clear()
...
func _adjust_pool_size() -> void:
	print("→ _adjust_pool_size pool=" + str(_free_nodes.size()) + " row_data=" + str(_row_data.size()) + " @ " + str(Time.get_ticks_msec()))
	print_stack()
	## 确保空闲池足够
